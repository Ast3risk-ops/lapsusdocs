...
icon: "file-pen"
iconType: "duotone"
description: "Customize your client with settings.json"
---
...
| `database`       | `sqlite://database.sqlite` | The path to an SQLite database file, starting with <code>sqlite://</code><br /><br /> <Info>The file will be created if it does not exist.</Info> |
...
<Tip>`new` entries here are incremented by 1 (ex. `new1`, `new2`, etc). We will use `new1` for reference, but it can be `new(any number)` to create new news entries.</Tip>

| Option             | Default | Description                                                   |
| ------------------ | ------- | ------------------------------------------------------------- |
| `new1_type`        |         | News type<br /><br />Can be whatever you want              |
| `new1_title`       |         | Title of news post                                            |
| `new1_message`     |         | Content of news post                                          |
| `new1_img`         |         | Cover image for news post <br /><br />Can be a path or URL |
...
| Option | Default | Description                                                                        |
| ------ | ------- | ---------------------------------------------------------------------------------- |
| `type` | `MB`    | Unit used to show resources <br /><br /> Can be: <code>MB</code>, <code>GB</code> |

## `website`

| Option   | Default | Description                                                                                                                                                                                                                                                              |
| -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `port`   | `8000`  | Port used to access Lapsus<br /><br />⚠️ This is HTTP-only. To set up HTTPS, please set up [a reverse proxy](/client/reference/reverse-proxy). |
...
| Option   | Default | Description          |
| -------- | ------- | -------------------- |
| `purger` | `true`  | ⚠️ Depreceated |
| `getip`  | `true`  | ⚠️ Depreceated |

### `webhook`

| Option      | Default | Description                                |
| ----------- | ------- | ------------------------------------------ |
| `auditlogs` | `false` | Send audit logs through the Discord webhook? |

### `api` (client)

<Warning>This is different from the _other_ [`api`](#api).</Warning>

| Option    | Default  | Description                 |
| --------- | -------- | --------------------------- |
| `enabled` | `false`  | Enables webhooks            |
| `code`    | `lapsus` | <Warning>Do not change this.</Warning> |

### `j4r`

<Info>For a guide on all this, see [J4R](/client/reference/discord#j4r-join-4-rewards).</Info>
...
<Tip>You can have multiple ad blocks within this section. See the example config for details.</Tip>

| Option   | Default | Description                                                                                                                |
| -------- | ------- | -------------------------------------------------------------------------------------------------------------------------- |
| `name`   |         | The name of the Discord server                                                                                             |
| `invite` |         | The invite link                                                                                                            |
| `id`     |         | The server's Guild ID |
...
| Option    | Default | Description                                                                                                                 |
| --------- | ------- | --------------------------------------------------------------------------------------------------------------------------- |
| `enabled` | `true`  | Is automatic guild joining enabled? <br /><br />(OAuth2 app joins the guild on the user's behalf during login)          |
